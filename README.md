...
| 模块 | 说明 |
|------|------|
| 🧩 **多格式导入** | 内置 `a`（Cubism 4 解包文件夹）、`b`（Steam Workshop `.lpk` 加密包）两个示例；支持拖拽 **模型文件夹 / 多文件 / `.lpk` / 背景图片**。`.lpk` 在浏览器内**本地解密**，无需任何后端。 |
...
│   └── jszip.min.js
├── assets/models/{a,b}/    # 解包后的示例模型
├── b/                      # 原始 .lpk 加密包 + config.json（用于“实时解密”演示）
...
- **文件夹模型**：包含 `*.model3.json` / `model.json`（或本项目示例的 `model0.json`）及其贴图、physics、表情等。拖入或用「模型文件夹」选择即可，资源通过 blob URL 在内存中加载。
...
- 示例预设「特莉波卡 · .lpk 实时解密」会现场抓取 `b/3376576902.lpk` 解密渲染，用于演示该能力。

> 解密算法移植自 [LpkUnpacker](LpkUnpacker/)，并通过 `tools/test_lpk.cjs` 验证与 Python 解包结果**逐字节一致**。
...
```bash
node tools/test_lpk.cjs     # 校验浏览器端 .lpk 解密与 Python 结果逐字节一致
node tools/smoke.cjs        # 无头 Chrome 加载页面，检查渲染与运行时错误
node tools/smoke2.cjs       # 驱动 .lpk 解密 / 表情 / 天气 / 滤镜 / 背景 等交互
```

`smoke` 系列通过 DevTools 协议在真实 WebGL 环境中加载并截获控制台错误 —— 当前全部通过、零异常。
